...
function answerCard(playerMon,aiMon,duel,perspective="opponent"){
  ...
  return `<article class="answer-card robust-answer-card">
...
function recommendationAnswerCard(row){
  ...
  return `<article class="answer-card player-answer robust-answer-card"><div class="answer-flow"><div><span>相手AI</span>${spriteToken(opponent,'option')}</div><b>⇩</b><div><span>あなたの回答</span>${spriteToken(player,'option')}</div></div><div class="answer-score"><strong>${row.duel.score100}/100</strong><span>${grade.label}</span>${compactScenarioStrip(row.duel)}</div></article>`;
}
