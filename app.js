...
function renderPokemonOptions(query){
  ...
  root.replaceChildren(...ids.map(id=>{
    ...
    button.innerHTML=`<span class="pokemon-rank">${p.rank?`#${p.rank}`:(p.metaFeatured?"注目":"—")}</span>${pokemonAvatar(p,"option")}<span class="pokemon-option-main"><strong>${escapeHtml(p.name)}</strong>${p.metaFeatured?'<span class="meta-inline">環境注目</span>':""}<small>CP ${p.cp}・${escapeHtml(moveLabel(p))}</small><span class="type-row">${typeChips(p.types)}</span></span>${duplicate?'<span class="duplicate-label">登録済み</span>':""}`;
    return button;
  }));
...
function matchupGrade(duel){
  ...
  if(duel.wins===1)return {label:"不利寄り",cls:"bad"};
...
function recommendationAnalysis(line){
  const rows=state.opponentRoster.map((oid,opponentIndex)=>{
    ...
    const solidAnswers=candidates.filter(x=>x.duel.wins>=2);
  ...
  const memberCoverage=line.map(playerIndex=>{
    ...
    const strong=state.opponentRoster.map((oid,opponentIndex)=>({opponent:POKEMON[oid].name,duel:headToHeadBySlots(playerIndex,opponentIndex)})).filter(x=>x.duel.wins>=2).sort((a,b)=>b.duel.score-a.duel.score);
    const weak=state.opponentRoster.map((oid,opponentIndex)=>({opponent:POKEMON[oid].name,duel:headToHeadBySlots(playerIndex,opponentIndex)})).filter(x=>x.duel.wins===0).sort((a,b)=>a.duel.score-b.duel.score);
  ...
  const leadBad=rows.filter(x=>headToHeadBySlots(leadIndex,x.opponentIndex).wins===0).map(x=>x.opponent);
  const warnings=[];
  if(heavy.length)warnings.push(`${heavy.map(x=>x.opponent).join("・")}は、この3体の誰を当てても同数シールドで2勝以上できないため重いです。`);
  ...
  if(leadBad.length)warnings.push(`先発${leadName}は${leadBad.slice(0,4).join("・")}に0勝3敗なので、初手対面では即引き候補です。`);
  if(!warnings.length)warnings.push("相手6体すべてに2勝以上できる回答があり、明確な穴は小さい選出です。");
...
function renderRecommendations(results,currentEstimate=null){
  ...
  panel.innerHTML=`<div class="recommendation-heading"><div><p class="step">SELECTION COACH</p><h3>勝ち筋が太い選出候補 ✨</h3></div><span class="status-chip">全60候補を比較</span></div><p class="recommendation-intro">あなたの60通りを対面データで一次比較し、上位候補を相手の60通り（先発を区別、控え順は同一）へ各3回、合計180試合で再検証しています。表示％は大会の実使用率ではありません。</p>`;
  ...
  results.forEach((result,rank)=>{
    ...
    const heavyHtml=analysis.heavy.length
      ...
      : '<li>相手6体すべてに、同数シールドで2勝以上できる回答があります。</li>';
    const coverageHtml=analysis.memberCoverage.map(member=>{
      const targets=member.strong.length?member.strong.map(x=>`${escapeHtml(x.opponent)}（${x.duel.wins}勝${x.duel.losses}敗）`).join("・"):"明確な有利対面なし";
